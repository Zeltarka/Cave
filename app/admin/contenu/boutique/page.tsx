// app/admin/contenu/boutique/page.tsx
"use client";
...
function BoutiqueEditor() {
    ...
    const mettreAJourProduit = (index: number, champ: keyof Produit, valeur: any) => {
        setContenu(prev => {
            if (!prev) return null;
            const newProduits = [...prev.produits];
            newProduits[index] = { ...newProduits[index], [champ]: valeur };
                ...
                <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <h1 className="text-2xl font-bold text-[#24586f]">Éditer Boutique</h1>
                        </div>
                ...
                <div className="space-y-6">
                    ...
                    {contenu.produits.map((produit, index) => (
                        <div key={index} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            <h3 className="text-lg font-semibold text-[#24586f] mb-4">Produit {index + 1}</h3>
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-2">Nom</label>
                                    <input type="text" value={produit.nom} onChange={(e) => mettreAJourProduit(index, "nom", e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#24586f]" />
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-2">Prix (€)</label>
                                    <input type="number" step="0.01" value={produit.prix} onChange={(e) => mettreAJourProduit(index, "prix", parseFloat(e.target.value) || 0)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#24586f]" />
                                </div>
                                <div>
...
export default function AdminBoutiquePage() {
    return <AdminGuard><BoutiqueEditor /></AdminGuard>;
}