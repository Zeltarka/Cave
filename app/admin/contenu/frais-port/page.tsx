...
function FraisPortEditor() {
    ...
    // Le maximum de bouteilles autorisé = la tranche la plus haute
    const maxActuel = frais.length > 0 ? Math.max(...frais.map(f => f.bouteilles_min)) : 0;

    const mettreAJourFrais = (id: string, champ: keyof FraisPort, valeur: number) => {
        setFrais(prev => prev.map(f => f.id === id ? { ...f, [champ]: valeur } : f));
    };

    const ajouterTranche = () => {
        setFrais(prev => [
            ...prev,
            { id: `new-${Date.now()}`, bouteilles_min: maxActuel + 6, frais: 0 },
        ]);
    };
    ...
    const sauvegarder = async () => {
        // Valider : pas de doublons sur bouteilles_min
        const valeurs = frais.map(f => f.bouteilles_min);
        const doublons = valeurs.filter((v, i) => valeurs.indexOf(v) !== i);
        if (doublons.length > 0) {
            afficherMessage(`Valeurs en double : ${doublons.join(", ")} bouteilles`, "error");
            return;
        }
    ...
    const paliersDisponibles = [6, 12, 18, 24, 30, 36, 42, 48, 60, 72, 96];
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
                ...
                {/* Récapitulatif du maximum déduit */}
                {frais.length > 0 && (
                    <div className="bg-[#f1f5ff] border border-[#24586f]/30 rounded-xl p-4 flex items-center justify-between">
                        <div>
                            <p className="text-sm text-gray-600">
                                Maximum de bouteilles autorisé par commande
                                <span className="text-xs text-gray-400 ml-2">(déduit de la tranche la plus haute)</span>
                            </p>
                            <p className="text-2xl font-bold text-[#24586f] mt-1">
                                {maxActuel} bouteilles
                            </p>
                        </div>
                        <div className="flex flex-wrap gap-1.5 max-w-xs justify-end">
                            {Array.from({ length: maxActuel / 6 }, (_, i) => (i + 1) * 6).map(q => (
                                <span key={q} className="px-2 py-0.5 bg-white border border-[#24586f]/30 text-[#24586f] rounded text-xs font-medium">
                                    {q}
                                </span>
                            ))}
                        </div>
                ...
                <div className="space-y-4">
                    {frais
                        .slice()
                        .sort((a, b) => a.bouteilles_min - b.bouteilles_min)
                        .map((tranche, index) => (
                            <div key={tranche.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                                <div className="flex justify-between items-center mb-4">
                                    <h3 className="text-lg font-semibold text-[#24586f]">
                                        Tranche {index + 1}
                                        {tranche.bouteilles_min === maxActuel && (
                                    ...
                                    <button
                                        ...
                                        className="px-3 py-1 text-red-600 hover:bg-red-50 rounded text-sm disabled:opacity-40"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select
                                            ...
                                            onChange={e => mettreAJourFrais(tranche.id, "bouteilles_min", parseInt(e.target.value))}
                                            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#24586f]"
                                        >
                                            {paliersDisponibles.map(v => (
                ...
                >
                    + Ajouter une tranche ({maxActuel + 6} bouteilles)
                </button>
