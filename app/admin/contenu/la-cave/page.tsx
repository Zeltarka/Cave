// app/admin/contenu/la-cave/page.tsx
"use client";
...
function LaCaveEditor() {
    const [contenu, setContenu] = useState<LaCaveContenu>({ blocs: [] });
    ...
    const fetchContenu = async () => {
        try {
            const res = await fetch("/api/admin/contenu/la-cave");
    ...
    const mettreAJourItem = (blocIndex: number, itemIndex: number, nouvelItem: string) => {
        setContenu(prev => ({
            blocs: prev.blocs.map((bloc, i) => {
                if (i === blocIndex && bloc.items) {
                    return {
                        ...bloc,
                        items: bloc.items.map((item, j) => j === itemIndex ? nouvelItem : item)
                    };
                }
                return bloc;
            })
    ...
    const ajouterItemListe = (blocIndex: number) => {
        setContenu(prev => ({
            blocs: prev.blocs.map((bloc, i) => {
                if (i === blocIndex && bloc.items) {
                    return { ...bloc, items: [...bloc.items, ""] };
                }
                return bloc;
            })
        }));
    ...
    const supprimerItemListe = (blocIndex: number, itemIndex: number) => {
        setContenu(prev => ({
            blocs: prev.blocs.map((bloc, i) => {
                if (i === blocIndex && bloc.items) {
                    return { ...bloc, items: bloc.items.filter((_, j) => j !== itemIndex) };
                }
                return bloc;
            })
        }));
    ...
    const ajouterBloc = (type: "paragraphe" | "liste") => {
        const newBloc: Bloc = type === "liste"
            ? { type: "liste", items: [""] }
            ...
            <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        >
                            ...
                            {bloc.type === "liste" ? (
                                <div className="space-y-3">
                                    {bloc.items?.map((item, itemIndex) => (
                                        <div key={itemIndex} className="flex gap-2">
                                            ...
                                            <input
                                                type="text"
                                                value={item}
                                                onChange={(e) => mettreAJourItem(index, itemIndex, e.target.value)}
                                                className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#24586f]"
                                                placeholder="Élément de liste"
                                            />
                                            <button
                                                onClick={() => supprimerItemListe(index, itemIndex)}
                                    ...
                                    <button
                                        onClick={() => ajouterItemListe(index)}
                                        className="px-4 py-2 text-[#24586f] border border-[#24586f] rounded-lg hover:bg-[#24586f] hover:text-white transition-colors text-sm"
                                    >
                                        + Ajouter un élément
                                    </button>
                                </div>
                ...
                <div className="mt-8 grid grid-cols-2 gap-4">
                    <button
                        ...
                        className="px-6 py-4 border-2 border-dashed border-[#24586f] text-[#24586f] rounded-lg hover:bg-[#24586f] hover:text-white transition-colors font-medium"
                    >
                        + Ajouter un paragraphe
                    </button>
                    <button
                        onClick={() => ajouterBloc("liste")}
                        className="px-6 py-4 border-2 border-dashed border-[#24586f] text-[#24586f] rounded-lg hover:bg-[#24586f] hover:text-white transition-colors font-medium"
                    >
                        + Ajouter une liste
                    </button>
...
export default function AdminLaCavePage() {
    return (
        <AdminGuard>
            <LaCaveEditor />
        </AdminGuard>
