...
function CommandeDetailContent() {
    ...
    const changerStatut = async (nouveauStatut: string) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const errorData = await res.json();
                afficherMessage(`Erreur: ${errorData.error}`, "error");
            }
        } catch (err) {
    ...
    const supprimerCommande = async () => {
        ...
        afficherConfirmation(
            `Supprimer définitivement la commande #${commande.id} ? Cette action est irréversible.`,
            async () => {
                setDeleting(true);
                try {
                    const res = await fetch(`/api/admin/commandes/${commande.id}`, { method: "DELETE" });
                    if (res.ok) {
                        router.push("/admin/commandes");
                    } else {
                        const data = await res.json();
                        afficherMessage(data.error || "Erreur lors de la suppression", "error");
                        setDeleting(false);
                    }
                } catch (err) {
                    afficherMessage("Erreur de connexion au serveur", "error");
    ...
    const afficherMessage = (msg: string, type: "success" | "error" = "success", titre?: string) => {
        setModalType(type);
        setModalMessage(msg);
        setModalTitle(titre);
    ...
    const afficherConfirmation = (msg: string, onConfirm: () => void, label = "Confirmer", titre?: string, type: "success" | "error" | "info" = "info") => {
        setModalType(type);
        setModalMessage(msg);
        setModalTitle(titre);
    ...
    const envoyerCarte = async (ligneId: number, email: string) => {
        ...
        afficherConfirmation(
            `Envoyer cette carte cadeau à ${email} ?`,
            async () => {
                ...
                try {
                    const res = await fetch(`/api/admin/commandes/${commande.id}/send-email`, {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ ligneId, emailDestinataire: email }),
                    });
                    const data = await res.json();
                    if (res.ok) {
                        afficherMessage(data.message || `Carte envoyée à ${email}`, "success");
                        fetchCommande();
                    } else {
                        afficherMessage(data.error || "Erreur lors de l'envoi", "error");
                    }
                } catch (err) {
                    afficherMessage("Erreur lors de l'envoi de la carte", "error");
                } finally {
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl h-[90vh] flex flex-col">
                        <div className="flex justify-between items-center p-4 border-b">
                            ...
                            <button onClick={fermerPdfModal} className="text-gray-400 hover:text-gray-600">
                        ...
                        <div className="flex-1 overflow-hidden">
                            <iframe src={pdfUrl} className="w-full h-full" title="Carte cadeau PDF" />
                        </div>
                    </div>
                </div>
            )}
                        ...
                        <div className="flex items-end gap-4">
                            <div>
                                ...
                                <select
                                    ...
                                    className={`text-sm font-semibold rounded-lg px-4 py-2 border ${getStatutColor(commande.statut)} ${changingStatus || deleting ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
                                >
                                    {statuts.map((statut) => (
                                        <option key={statut} value={statut}>{getStatutLabel(statut)}</option>
                                    ))}
                                    <option disabled>──────────</option>
                                    <option value="supprimer">Supprimer la commande</option>
                                </select>
                                {changingStatus && <p className="text-xs text-gray-500 mt-1">Mise à jour...</p>}
                                {deleting && <p className="text-xs text-red-500 mt-1">Suppression...</p>}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-6">
                                            ...
                                            <div key={index} className="p-4 rounded-lg border border-gray-200 border-l-4 border-l-[#24586f]">
                                                <div className="flex justify-between items-start gap-4">
                                                    <div className="flex-1">
                                                        ...
                                                        {carteEnvoyee && (
                                                            <div className="mt-2 flex items-center gap-2 text-gray-600 text-sm border-l-4 border-l-[#24586f] pl-3">
                                                    ...
                                                    <div className="flex flex-col gap-2">
                                                        ...
                                                        >
                                                            {isGenerating ? (
                                                                ...
                                                                <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>Afficher PDF</>
                                                            )}
                                                        </button>
                                                        {carte.ligneId && (
                                                            <button
                                                                ...
                                                                disabled={isSending}
                                                                className="px-4 py-2 bg-white text-[#24586f] border-2 border-[#24586f] rounded-lg hover:bg-[#24586f] hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 whitespace-nowrap"
                                                            >
                                                                {isSending ? (
                                                                    <><svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg>Envoi...</>
                                                                ) : (
                                                                    <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>{carteEnvoyee ? "Renvoyer" : "Envoyer carte"}</>
                                                                )}
                                                            </button>
                                                        )}
                                                    </div>
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="text-base font-medium text-gray-900">{commande.prenom} {commande.nom}</p>
                                </div>
                                <div>
                        ...
                        {commande.source === "boutique_admin" && (
                            <div className="border border-gray-200 border-l-4 border-l-[#24586f] rounded-xl p-4 flex items-center gap-3">
                                <svg className="w-5 h-5 text-[#24586f] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <p className="text-sm font-medium text-gray-700">Commande créée en boutique par un administrateur</p>
                            </div>
                        )}

                        {/* Mode de livraison */}
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            <h2 className="text-lg font-semibold text-gray-900 mb-4">Mode de récupération</h2>
                            <div className="space-y-3">
                                <div>
                                    <p className="text-sm text-gray-500">Type</p>
                                    <p className="text-base font-medium text-gray-900">
                                        {commande.modeLivraison === "livraison" ? "Livraison à domicile" : "Retrait en boutique"}
                                    </p>
                                </div>
                                {commande.modeLivraison === "livraison" && commande.adresse && (
                                    <div>
                                        ...
                                        {commande.fraisPort > 0 && (
                                            <div className="mt-2 p-3 border border-gray-200 border-l-4 border-l-[#24586f] rounded-lg">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <p className="text-base font-medium text-gray-900">
                                        {commande.modePaiement === "boutique" ? "Paiement en boutique" : "Virement bancaire"}
                                    </p>
                                </div>
                                {commande.modePaiement === "boutique" && commande.datePassage && (
                                    <div>
                                        ...
                                        <p className="text-base font-medium text-gray-900">
                                            {new Date(commande.datePassage + "T00:00:00").toLocaleDateString("fr-FR", {
                                ...
                                {commande.modePaiement === "virement" && (
                                    <div className="p-3 border border-gray-200 border-l-4 border-l-[#24586f] rounded-lg">
                                        <p className="text-sm text-gray-700">Attendre la réception du virement avant de préparer la commande.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        {commande.commentaires && (
                            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                                <h2 className="text-lg font-semibold text-gray-900 mb-4">Commentaires du client</h2>
                                <p className="text-base text-gray-700 whitespace-pre-wrap">{commande.commentaires}</p>
                            </div>
                        )}
                            ...
                            <div className="space-y-3">
                                {commande.panier.map((produit, index) => {
                                    ...
                                    const nomAffiche = isCarteCadeau && produit.destinataire
                                        ...
                                        : produit.produit;
                                    return (
                                        <div key={index} className="flex justify-between items-start p-4 bg-gray-50 rounded-lg border-l-4 border-l-gray-300">
                                            <div className="flex-1">
                                                <p className="font-medium text-gray-900">{nomAffiche}</p>
                                                <p className="text-sm text-gray-500 mt-1">Quantité : {produit.quantite}</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-sm text-gray-500">{produit.prix.toFixed(2)} € / unité</p>
                                                <p className="font-semibold text-gray-900 mt-1">{(produit.prix * produit.quantite).toFixed(2)} €</p>
                                            </div>
                                        </div>
                                    );
                                })}
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-6">
                            ...
                            <div className="space-y-3 mb-6">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-gray-900">{commande.total.toFixed(2)} €</span>
                                </div>
                                {commande.fraisPort > 0 && (
                                    <div className="flex justify-between text-sm">
                                        <span className="text-gray-600">Frais de port</span>
                                        <span className="text-gray-900 font-medium">{commande.fraisPort.toFixed(2)} €</span>
                                    </div>
                                )}
                            </div>
                            <div className="pt-3 border-t border-gray-200">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-xl font-bold text-[#24586f]">{totalAvecPort.toFixed(2)} €</span>
                                </div>
                            </div>
                            <div className="mt-6 pt-6 border-t border-gray-200">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-sm text-gray-600">Statut actuel</span>
                                    <span className={`text-xs font-semibold rounded-full px-3 py-1 border ${getStatutColor(commande.statut)}`}>
                                        {getStatutLabel(commande.statut)}
                                    </span>
                                </div>
                                <p className="text-xs text-gray-500 mt-2">Modifiez le statut en haut de la page</p>
                            </div>
                        </div>
                    </div>
                </div>
