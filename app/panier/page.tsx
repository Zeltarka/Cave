...
export default function PanierPage() {
    ...
    const totalBouteilles = panier
        .filter(p => p.id === "champagne" || p.id === "rose")
        .reduce((sum, p) => sum + p.quantite, 0);
    ...
    const getQuantitesDisponibles = (produit: Produit) => {
        ...
        if (produit.id === "champagne" || produit.id === "rose") {
            const autresBouteilles = panier
                .filter(p => (p.id === "champagne" || p.id === "rose") && p.id !== produit.id)
                .reduce((sum, p) => sum + p.quantite, 0);
            const maxDispo = 24 - autresBouteilles;
            return [6, 12, 18, 24].filter(qty => qty <= maxDispo);
        }
        return [6, 12, 18, 24];
    };
    ...
    const changerQuantite = async (id: string, nouvelleQuantite: number) => {
        ...
        if (id === "champagne" || id === "rose") {
            const autresBouteilles = panier
                .filter(p => (p.id === "champagne" || p.id === "rose") && p.id !== id)
                .reduce((sum, p) => sum + p.quantite, 0);
            const nouveauTotal = autresBouteilles + nouvelleQuantite;
            if (nouveauTotal > 24) {
                nouvelleQuantite = Math.max(0, 24 - autresBouteilles);
            }
        }
    ...
    const validerCommande = async () => {
        ...
        try {
            const res = await fetch("/api/commandes/valider", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ client: commande, panier, total: totalAvecPort, fraisPort }),
            });
            const data = await res.json();
        ...
        <div className="p-4 sm:p-8 lg:p-12 flex justify-center">
            <div className="w-full max-w-5xl">
                ...
                {panierVide ? (
                    ...
                    <>
                        {/* ✅ Bandeau max 24 bouteilles — couleurs du site */}
                        {totalBouteilles >= 24 && (
                            <div className="mb-6 p-4 bg-[#f1f5ff] border-2 border-[#24586f] rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                                <p className="text-[#24586f] font-medium">
                                    Vous avez atteint le maximum de 24 bouteilles. Pour une commande plus importante, contactez-nous.
                                </p>
                                <Link
                                    href="/contact"
                                    className="flex-shrink-0 px-4 py-2 bg-[#24586f] text-white rounded-lg hover:bg-[#1a4557] transition-colors font-medium text-sm"
                                >
                                    Nous contacter
                                </Link>
                            </div>
                        ...
                        {!panierVide && (
                            <>
                                ...
                                <div className="text-center mt-6">
                                    <Link href="/la-cave" className="inline-block mr-4 px-6 py-3 border-2 border-[#24586f] text-[#24586f] rounded-lg hover:bg-[#24586f] hover:text-white transition-colors font-medium">
                        ...
                        {afficherCommande && !panierVide && (
                            <div id="formulaire-commande" className="mt-8 p-4 sm:p-6 border border-gray-300 rounded-lg bg-white shadow-md">
                                ...
                                <div className="mb-6">
                                    <label className="block text-gray-700 font-semibold mb-3">Mode de récupération *</label>
                                    <div className="space-y-3">
                                        <label className="flex items-center p-3 border border-gray-300 rounded cursor-pointer hover:bg-gray-50 transition-colors">
                                            <input type="radio" name="modeLivraison" value="retrait" checked={commande.modeLivraison === "retrait"} onChange={handleChange} className="w-4 h-4 text-[#24586f] focus:ring-[#24586f]" />
                                            <span className="ml-3">
                                        ...
                                        <label className="flex items-center p-3 border border-gray-300 rounded cursor-pointer hover:bg-gray-50 transition-colors">
                                            <input type="radio" name="modeLivraison" value="livraison" checked={commande.modeLivraison === "livraison"} onChange={handleChange} className="w-4 h-4 text-[#24586f] focus:ring-[#24586f]" />
                                            <span className="ml-3">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <input name="nom" placeholder="Nom *" value={commande.nom} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#24586f]" />
                                        <input name="prenom" placeholder="Prénom *" value={commande.prenom} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#24586f]" />
                                    </div>
                                    <input name="email" type="email" placeholder="Adresse email *" value={commande.email} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#24586f]" />
                                    <input name="telephone" type="tel" placeholder="Numéro de téléphone (optionnel)" value={commande.telephone} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#24586f]" />

                                    {commande.modeLivraison === "livraison" && (
                                        <>
                                            <input name="adresse" placeholder="Adresse *" value={commande.adresse} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#24586f]" />
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                <input name="codepostal" placeholder="Code Postal *" value={commande.codepostal} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#24586f]" />
                                                <input name="ville" placeholder="Ville *" value={commande.ville} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#24586f]" />
                                            </div>
                                        </>
                                    )}

                                    <textarea name="commentaires" placeholder="Commentaires (optionnel)" value={commande.commentaires} onChange={handleChange} rows={4} className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#24586f] resize-vertical" />
                                </div>

                                <div className="mt-6">
                                    <label className="block text-gray-700 font-semibold mb-3">Mode de paiement *</label>
                                    <div className="space-y-3">
                                        <label className="flex items-start p-3 border border-gray-300 rounded cursor-pointer hover:bg-gray-50 transition-colors">
                                            ...
                                            <span className="ml-3">
                                                ...
                                                <span className="text-sm text-gray-600 block">Vous recevrez les informations bancaires par email. La commande sera traitée après réception du paiement.</span>
                                            </span>
                                        </label>
                                        <label className="flex items-start p-3 border border-gray-300 rounded cursor-pointer hover:bg-gray-50 transition-colors">
                                            ...
                                            <span className="ml-3">
                                                ...
                                                <span className="text-sm text-gray-600 block">Vous paierez directement en boutique lors de la récupération de votre commande.</span>
                                            </span>
                                        </label>
                                    </div>
                                </div>
                                ...
                                {(commande.modeLivraison === "retrait" || commande.modePaiement === "boutique") && (
                                    <div className="mt-4">
                                        <label className="block text-gray-700 font-semibold mb-2">Date de passage en boutique souhaitée *</label>
                                        <input
